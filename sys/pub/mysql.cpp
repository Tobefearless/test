...
bool mysql::login(QString userName, QString password)
{
       ...
       if(query.exec(sql)){
           while (query.next()) {
               qDebug()<<"账号"<<query.value("user").toString()
...
bool mysql::toDB()
{
    QString   sql;
        bool result;
        sql = QString("SELECT id from `toapp` ");
        QSqlQuery  query(db);
...
bool mysql::WORKwayToDB(WORK_WAY_TYEP data,QString status)
{
    ...
    QByteArray work_way = Convert_struct((char*)&data,sizeof(WORK_WAY_TYEP));
    QString id = QString(data.id);
...
bool mysql::GetMode()
{
   ...
   // for( uint8_t i = 0 ; i < DEV ; i++ ){
        ...
        sql = QString(R"( SELECT * FROM `update_all_table` WHERE id = '%1' )")
                .arg(QString(global_data::id[0]));

        if(query.exec(sql)){
            while (query.next()){

                    uint8_t id = 0; //query.value(1).toString().right(3).toInt() - 1;
                    if( global_data::Mode_Lock[id] == Write_Unlock ){
                            if( query.value("run_way_from_mcu_status").toInt() ){
                                    bool ok;
                                ...
                                Mode_Clear(QString(global_data::id[0]));
                                return true;
                            }
                    ...
                    if( GET_DEV_STATUS_HEAD == head && GET_DEV_STATUS_TAIL == tail){
                        ...
                        uint16_t crc = CRC16_2_modbus((uint8_t*)&work_Contrl_mode,sizeof(WORK_WAY_TYEP)-2);
                        if( work_Contrl_mode.crc == crc ){
                                //uint8_t temp = global_data::control_Mode[id];
...
bool mysql::GetTable(QString device)
{
    //接收标志位
        //static bool rcv_flag[DEV] = {false};
        static uint16_t pre_crc;
        QString sql;
        ...
        sql = QString(R"( SELECT * FROM `update_all_table` WHERE id = '%1' )").arg(device);
        if(query.exec(sql)){
            while (query.next()) {
...
QStringList mysql::GetArea()
{
    QSqlQuery query(db);
    QStringList list;
    //在数据库插入空行
    QString sql = QString (" SELECT area FROM user_Maneger ");
    if( query.exec(sql) ){
        while (query.next()) {
            if( query.value(0).toString().size() > 8 ){
...
QStringList mysql::GetDevice(QString area)
{
    QSqlQuery query(db);
    QStringList list;
    //在数据库插入空行
    QString sql = QString (R"( SELECT id FROM update_all_table WHERE id LIKE '%%1%' )").arg(area);
    if( query.exec(sql) ){
        while (query.next()) {
                list.append(query.value(0).toString());
        }
...
QStringList mysql::Get_AbNormal_Device()
{
    QSqlQuery query(db);
    QStringList list;
    //在数据库插入空行
    QString sql = QString (R"( SELECT id,dev_status FROM update_all_table )");
    if( query.exec(sql) ){
        while (query.next()) {
                if( !query.value(1).toInt() ){
                    list.append( query.value(0).toString()  + "    离线");
                }
        }
    }
