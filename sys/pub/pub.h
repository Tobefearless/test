...
#define Recive_Topic "dev/recive/"
...
#define ID_LENGTH 20
//单片机上传的数据
#define TO_APP_HEAD_s "a2"
#define _485_CMD_HEAD_s "a4"

#define FILENAME "C:/Windows/SCGX230413001"
#define RecordFILE "C:/Windows/Historical_data_sheet"
...
typedef enum{
    ...
    //传感器数值
    SENSOR_DATA_HEAD = 0xC1,
    SENSOR_DATA_TAIL = 0xC2
}HEAD_TAIL_CMD;
...
typedef struct {
    ...
    uint16_t nh3;
    uint16_t cio2;
    SINGLE_SENSOR_Typedef single_co[4];
    SINGLE_SENSOR_Typedef single_co2[4];
    SINGLE_SENSOR_Typedef single_nh3[4];
    SINGLE_SENSOR_Typedef	single_cio2[4];
    uint8_t tail;
    uint16_t crc;
}FROM_SENSOR_TYPE;
...
public:
    ...
    //房间的ID号
    static char id[DEV][20];
    //日期更新了的标志位
    static bool update_flag;
    //房间参数表是否更新完毕
    static bool rcv_ok;
    //心跳包发送标志位
    static bool Hear_flag;
    //房间发送成功
    static bool room_flag[DEV];
    //房间网上发送成功
    static bool Send_onine_flag[DEV];
    //设备联网与未联网
    static bool on_line;
    //参数表更新到TXT文本标志位
    static bool update_RoomFlag[DEV];
    //心跳包计数
    static uint8_t heart_beat_count[DEV];
    //模式选择
    static uint8_t control_Mode[DEV];
    //更新的参数表
    static UPDATE_ALL_TABLE_TYPEDEF update_allTable[DEV];
    //软件启动的天数
    static QDate sensor_Date;
    //数据库 串口互斥
    static bool Mutual_exclusion[DEV];
    //模式锁
    static bool Mode_Lock[DEV];
    //更新的模式
    static WORK_WAY_TYEP work_way[DEV];
    //开始状态接收标志位
    static bool mode_rcv_flag;
    //执行的状态
    static EXECUTE_DATE_Typedef execute_date[DEV];

    //默认参数标志位
    static bool Default_flag;
    //mqtt状态
    static bool mqtt_status;
    ...
    static double coJust;
    static double co2Just;
    static double so2Just;
    static double nh3Just;
};
