...
void home::State_assignment()
{
    ...
    for( uint8_t i = 0 ; i < DEV ; i++ ){
        Device[i]->Device_Name->setText(
                    QString().sprintf(R"(<html><head/><body><p><span style=" font-size:18pt; font-weight:600;">%s</span></p></body></html>)",global_data::id[i]));
    ...
    for( uint8_t i = 0; i < DEV ; i++ ){
        ...
        */
        ...
        for( uint8_t i = 0 ; i < DEV ; i++ ){
            ...
            sensor->other_sensor_table->setItem(0 + i * 4,3,
                                          new QTableWidgetItem(
                                                    QString::number(global_data::sensor_just_data[i].coJust,'f',1)));
            sensor->other_sensor_table->setItem(1 + i * 4,3,
                                          new QTableWidgetItem(
                                                    QString::number(global_data::sensor_just_data[i].co2Just,'f',1)));
            sensor->other_sensor_table->setItem(2 + i * 4,3,
                                          new QTableWidgetItem(
                                                    QString::number(global_data::sensor_just_data[i].cio2Just,'f',1)));
            sensor->other_sensor_table->setItem(3 + i * 4,3,
                                          new QTableWidgetItem(
                                                    QString::number(global_data::sensor_just_data[i].nh3Just,'f',1)));
        }
...
void home::Sensor_show(uint8_t room)
{
    ...
    if( flag )
    {
        Device[room]->Sensor_status->setText(R"(<html><head/><body><p><span style=" font-size:16pt; font-weight:600; color:#00ff7f;">传感器正常    </span></p></body></html>)");
    }
    else
    {
        Device[room]->Sensor_status->setText(R"(<html><head/><body><p><span style=" font-size:16pt; font-weight:600; color:#ff0000;">传感器异常    </span></p></body></html>)");
    }
...
void home::show_Sensor_Data(FROM_SENSOR_TYPE from_sensor)
{
    int id = -1;//QString(Recive_485_cmd.id).right(3).toInt()-1;
    id = ID_lookup(QString(from_sensor.id));
    if( id == -1 ) return ;

    uint16_t MQ_X_NUM_nh3 = from_sensor.nh3;

    float MQ_X_nh3=  MQ_X_NUM_nh3  * ( 5.0 / 4096);//数字量转换为电压值显示，12为ADC，所以除以4096;0~3.3V 线性转换为0~5V，所以除以0.66


        /*此处可增加气体含量与模拟电压的计算公式，进行气体含量计算*/
    double  NH3_ppm    =   NH3Calculator(MQ_X_nh3);
    double  CIO2_ppm;

         //测量范围限值
    if(NH3_ppm<=0){NH3_ppm=0;  CIO2_ppm = NH3_ppm;}
    if(NH3_ppm>=500){NH3_ppm=500;CIO2_ppm = NH3_ppm;}
    else  { CIO2_ppm = NH3_ppm / 2;}


    QString coText     = QString::number(from_sensor.co + global_data::sensor_just_data[id].coJust,'f',2);
    QString co2Text    = QString::number(from_sensor.co2 + global_data::sensor_just_data[id].co2Just,'f',2);
    QString Nh3Text    = QString::number(NH3_ppm + global_data::sensor_just_data[id].nh3Just ,'f',2);
    QString cio2Text   = QString::number(CIO2_ppm + global_data::sensor_just_data[id].cio2Just ,'f',2);

    Device[id]->co->setText(QString().
    sprintf(R"(<html><head/><body><p><span style=" font-size:24pt; font-weight:600; color:#ffffff;">%sppm</span></p></body></html>)",coText.toLatin1().data()));

    Device[id]->co2->setText(QString().sprintf(R"(<html><head/><body><p><span style=" font-size:22pt; font-weight:600; color:#ffffff;">%sppm</span></p></body></html>)",co2Text.toLatin1().data()));
    Device[id]->nh3->setText(QString().sprintf(R"(<html><head/><body><p><span style=" font-size:22pt; font-weight:600; color:#ffffff;">%sppm</span></p></body></html>)",Nh3Text.toLatin1().data()));
    Device[id]->cio2->setText(QString().sprintf(R"(<html><head/><body><p><span style=" font-size:22pt; font-weight:600; color:#ffffff;">%sppm</span></p></body></html>)",cio2Text.toLatin1().data()));

    sensor->other_sensor_table->setItem(0 + id * 4,2,
                                  new QTableWidgetItem(QString::number(from_sensor.co)));
    sensor->other_sensor_table->setItem(1+ id * 4,2,
                                  new QTableWidgetItem(QString::number(from_sensor.co2)));

    sensor->other_sensor_table->setItem(2+ id * 4,2,
                                  new QTableWidgetItem(QString::number(CIO2_ppm,'f',2)));

    sensor->other_sensor_table->setItem(3+ id * 4,2,
                                  new QTableWidgetItem(QString::number(NH3_ppm,'f',2)));

    sensor->other_sensor_table->setItem(0+ id * 4,4,
                                  new QTableWidgetItem(coText));
    sensor->other_sensor_table->setItem(1+ id * 4,4,
                                  new QTableWidgetItem(co2Text));
    sensor->other_sensor_table->setItem(2+ id * 4,4,
                                  new QTableWidgetItem(cio2Text));
    sensor->other_sensor_table->setItem(3+ id * 4,4,
                                  new QTableWidgetItem(Nh3Text));

    for( uint8_t i = 0 ; i < 4 ; i++ ){
        sensor->other_sensor_table->item(i,1)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
        sensor->other_sensor_table->item(i,3)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
    }
...
void home::room_show(FROM_DB_Typedef data, uint8_t room )
{
    ...
    if( data.status == ONLINE){
          Device[room]->Equipment->setText(Set_font_Style("18pt"," ","00ff00","设备在线"));
          //心跳包计数
          ...
          Device[room]->Equipment->setText(Set_font_Style("18pt"," ","ff0000","设备离线"));
    ...
    Device[room]->temp_center->setText(Set_font_Style("16pt"," ","ffffff",data.temp_center.toUtf8().data()));
    Device[room]->humi_center->setText(Set_font_Style("16pt"," ","ffffff",data.humi_center.toUtf8().data()));
    ...
    Device[room]->Temperature->setStyleSheet("font-size:50pt; font-weight:600; color:#ffbeaa;");

    Device[room]->Humidity->setText(data.humi);
    Device[room]->Humidity->setStyleSheet("font-size:50pt; font-weight:600; color:#ffbeaa;");
}
...
void home::__485_rcv_data(QByteArray data)
{
     ...
     else if( SENSOR_DATA_HEAD == head && SENSOR_DATA_TAIL == tail){
         FROM_SENSOR_TYPE from_sensor;
         memcpy((char*)&from_sensor,rcv,sizeof(FROM_SENSOR_TYPE));

         crc = CRC16_2_modbus((uint8_t*)&from_sensor,sizeof(FROM_SENSOR_TYPE) - 2);

         if( crc == from_sensor.crc ){
                show_Sensor_Data(from_sensor);
         }
     }
     serial->flush();
...
//    sensor_heart_counts++;
//    if( sensor_heart_counts > 10 ){
//        sensor_heart_counts = 0;
...
{
    //qDebug()<<"我在发心跳包"
    char temp_id[2][20] = {"SXZC220808001","SXZC220808002"};
    ...
    //qDebug()<<"阶段"<<global_data::stage[0];
            ...
            if( count >= DEV )
                ...
                memcpy(Send_485_cmd.id,temp_id[count],sizeof(Send_485_cmd.id));
