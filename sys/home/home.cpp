...
void home::State_assignment()
{
        ...
        sensor->other_sensor_table->setItem(0,2,
                                      new QTableWidgetItem(
                                                QString::number(global_data::coJust,'f',1)));
        sensor->other_sensor_table->setItem(1,2,
                                      new QTableWidgetItem(
                                                QString::number(global_data::co2Just,'f',1)));
        sensor->other_sensor_table->setItem(2,2,
                                      new QTableWidgetItem(
                                                QString::number(global_data::so2Just,'f',1)));
        sensor->other_sensor_table->setItem(3,2,
                                      new QTableWidgetItem(
                                                QString::number(global_data::nh3Just,'f',1)));
...
void home::update_table_485()
{
    ...
    hear_beat++;
    qDebug()<<"hear_beat"<<hear_beat<<"global_data::heart_beat_count"
...
void home::Send_Heart()
{
    ...
    if( !global_data::Hear_flag )
        ...
        for( uint8_t i = 0 ; i < DEV ; i++ ){
                ...
                memcpy(Send_485_cmd.id,global_data::id[0],sizeof(Send_485_cmd.id));
                Send_485_cmd.cmd = DEV_ALIVE_PACK;
                Send_485_cmd.crc = CRC16_2_modbus((uint8_t *)&Send_485_cmd,sizeof(__485_CMD_Type)-2);
                serial->write((char*)&Send_485_cmd,sizeof(__485_CMD_Type));
...
void home::__485_rcv_data(QByteArray data)
{
    ...
    if( SENSOR_DATA_HEAD == head && SENSOR_DATA_TAIL == tail){
        FROM_SENSOR_TYPE from_sensor;
        qDebug()<< "sizeof(FROM_SENSOR_TYPE)"<<sizeof(FROM_SENSOR_TYPE);
        ...
        if( crc == from_sensor.crc ){
            ...
            uint16_t MQ_X_NUM = from_sensor.nh3;
            float MQ_X=  MQ_X_NUM  * ( 5.0 / 4096);//数字量转换为电压值显示，12为ADC，所以除以4096;0~3.3V 线性转换为0~5V，所以除以0.66

                /*此处可增加气体含量与模拟电压的计算公式，进行气体含量计算*/
            double  NH3_ppm=410.74*MQ_X*MQ_X*MQ_X*MQ_X*MQ_X-2718.5*MQ_X*MQ_X*MQ_X*MQ_X+7015.6*MQ_X*MQ_X*MQ_X-8747.9*MQ_X*MQ_X+5271.7*MQ_X-1228.7;

                 //测量范围限值
            if(NH3_ppm<=0){NH3_ppm=0;}
            if(NH3_ppm>=500){NH3_ppm=500;}

            QString coText = QString::number(from_sensor.co + global_data::coJust,'f',1);
            QString co2Text = QString::number(from_sensor.co2 + global_data::co2Just,'f',1);
            QString Nh3Text = QString::number(NH3_ppm + global_data::nh3Just ,'f',2);

            Abnormal_Widegt->item(1)->setText(coText + "ppm");
            Abnormal_Widegt->item(3)->setText(co2Text + "ppm");
            Abnormal_Widegt->item(5)->setText(Nh3Text + "ppm");

            sensor->other_sensor_table->setItem(0,1,
                                          new QTableWidgetItem(QString::number(from_sensor.co)));
            sensor->other_sensor_table->setItem(1,1,
                                          new QTableWidgetItem(QString::number(from_sensor.co2)));

            sensor->other_sensor_table->setItem(2,1,
                                          new QTableWidgetItem(QString::number(NH3_ppm,'f',1)));

            sensor->other_sensor_table->setItem(0,3,
                                          new QTableWidgetItem(coText));
            sensor->other_sensor_table->setItem(1,3,
                                          new QTableWidgetItem(co2Text));

            sensor->other_sensor_table->setItem(2,3,
                                          new QTableWidgetItem(Nh3Text));

            sensor->other_sensor_table->item(0,1)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
            sensor->other_sensor_table->item(1,1)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
            sensor->other_sensor_table->item(2,1)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
            sensor->other_sensor_table->item(0,3)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
            sensor->other_sensor_table->item(1,3)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
            sensor->other_sensor_table->item(2,3)->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
