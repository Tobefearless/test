#include "co_parenting1.h"
#include "ui_co_parenting1.h"
#include "pub/controldelegate.h"
#include <QProgressDialog>
Co_parenting::Co_parenting(uint8_t room, QWidget *parent) :
    QMainWindow(parent),Myroom(room),
    ui(new Ui::Co_parenting)
{
    ui->setupUi(this);


    this->setWindowTitle("共育参数设置");
    //接收设置完成的信号

    dayChangeFlag = false;
    WriteTo_Table();
    initHeadFrame(ui->tableWidget_head,"共育时间");
    Parameter_setting();
    dayChangeFlag = true;
    //将共育参数写入表格
    //WriteTo_Table();
    ui->title->setText(QString(global_data::id[Myroom]) + + "共育参数设置");
...
void Co_parenting::Parameter_setting()
{
...
void Co_parenting::DataTo_txt(uint8_t stage)
{
        qDebug()<<"写进文本";
        QString strAll;
        QStringList strList;

        QString filename = QString(FILENAME)  + "/coparaent/coparaent-" + QString::number(Myroom+1) + ".txt";
...
void Co_parenting::setDayFlag(bool flag)
{
    dayChangeFlag = flag;
}

void Co_parenting::closeEvent(QCloseEvent *event)
{
    qDebug()<<"共育窗口关闭了"<<Myroom<<event->ActionChanged;
    Change_row.clear();
    execut_stage = false;
    emit Table_UnCLOCK();
}

void Co_parenting::WriteTo_Table()
{
    uint8_t row = 0;
    QString head = "****",line1;
    QStringList list;
    ...
    if( readfile.open((QIODevice::ReadOnly | QIODevice::Text))){
        ...
        while ( !line1.isNull()) {
            if( line1.contains(head)){
                ...
                ui->tableWidget->item(row,0)->setBackground(global_data::date_Color);
                ui->tableWidget->item(row,1)->setBackground(global_data::date_Color);
                ui->tableWidget->item(row,2)->setBackground(global_data::day_Color);

                line1 = stream.readLine();
                list = line1.split("/");
                ui->tableWidget->setItem(row,3,new QTableWidgetItem(list[0]));
                ui->tableWidget->setItem(row,4,new QTableWidgetItem(list[1]));
                ui->tableWidget->item(row,3)->setBackground(global_data::temp_Color);
                ui->tableWidget->item(row,4)->setBackground(global_data::humi_Color);
                ...
                for( uint8_t i = 0 ,tempIndex = 3; i < 8 ; i++,tempIndex++ ){
                    ...
                    ui->tableWidget->item(row,tempIndex*2-1)->setBackground(global_data::breath_Color);
                    ui->tableWidget->item(row,tempIndex*2)->setBackground(global_data::breath_Color);
                }
                ...
                for( uint8_t i = 0 ,tempIndex = 11; i < 3 ; i++ ,tempIndex++){
                    ...
                    ui->tableWidget->setItem(row,tempIndex*2-1,new QTableWidgetItem(list[0]));
                    ui->tableWidget->setItem(row,tempIndex*2,new QTableWidgetItem(list[1]));
                    ui->tableWidget->item(row,tempIndex*2-1)->setBackground(global_data::sensitive_Color);
                    ui->tableWidget->item(row,tempIndex*2)->setBackground(global_data::sensitive_Color);
                }
                ...
                for( uint8_t i = 0 ,tempIndex = 14; i < 4 ; i++ ,tempIndex++){
                    ...
                    ui->tableWidget->item(row,tempIndex*2-1)->setBackground(global_data::disinfect_Color);
                    ui->tableWidget->item(row,tempIndex*2)->setBackground(global_data::disinfect_Color);
                }
                ...
                for( uint8_t i = 0 ,tempIndex = 18; i < 3 ; i++ ,tempIndex++){
                    ...
                    ui->tableWidget->setItem(row,tempIndex*2-1,new QTableWidgetItem(list[0]));
                    ui->tableWidget->setItem(row,tempIndex*2,new QTableWidgetItem(list[1]));
                    ui->tableWidget->item(row,tempIndex*2-1)->setBackground(global_data::illumina_Color);
                    ui->tableWidget->item(row,tempIndex*2)->setBackground(global_data::illumina_Color);
                }
            }
            row++;
            line1 = stream.readLine();
...
void Co_parenting::Tbale_Send(uint8_t row)
{
...
//            uint8_t temp = i % table_row;
    ...
    if( global_data::on_line ){
        ...
        if( !status ){
            ...
            //QMessageBox::information(NULL, "参数表", "数据发送成功", "确定");
        }

        DB.Change_Save(Myroom);
}

bool Co_parenting::Device_Change(uint8_t room, WORK_WAY_STATUS way)
{
    sleep(1000);
...
void Co_parenting::Message_alert(QString title, QString text, QMessageBox::Icon hint)
{
...
void Co_parenting::isExecuteTime()
{
    uint8_t table_row = ui->tableWidget->rowCount();
    QDate curDate = QDate::currentDate(),staDate,endDate;
    QString year = curDate.toString("yyyy/");
...
void Co_parenting::on_button_save_clicked()
{
    qDebug()<<"数据开始解析";
    ...
    if( ok ){
        if( text == "888888"){
            ...
            if( global_data::execute_date[Myroom].run_stage >= 0 ){
                ...
                if( global_data::control_Mode[Myroom] != GONGYU_STA){
                    bool status =  Device_Change(Myroom, GONGYU_STA);
                    global_data::Mode_Lock[Myroom] = Write_lock;
                    if( status ){
                        Message_alert("提示","模式修改成功",QMessageBox::Information);
                    }
                    else{
                        QMessageBox::warning(NULL, "提示", "模式更改失败", "确定");
                    }
                }

            }
            else{
                QMessageBox::warning(NULL, "警告", "没有可执行的日期", "确定");
            }
        }
        else{
...
void Co_parenting::on_tableWidget_itemDoubleClicked(QTableWidgetItem *item)
{
...
void Co_parenting::on_butto_back_clicked()
{
    Change_row.clear();
    execut_stage = false;
    emit Table_UnCLOCK();
...
void Co_parenting::on_button_stop_clicked()
{
    qDebug()<<Myroom<<"号设备停止共育";
    ...
    if( ok ){
        if(text == "888888"){
            if( global_data::control_Mode[Myroom] != GONGYU_END ){
                bool status = Device_Change(Myroom, GONGYU_END);
                global_data::Mode_Lock[Myroom] = Write_lock;
                if( status )
                    Message_alert("提示","模式修改成功",QMessageBox::Information);
                else
                ...
                Message_alert("提示","设备已经停止",QMessageBox::Warning);
            }
...
void Co_parenting::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    if( item == nullptr )
        return ;
    if( !dayChangeFlag )
