...
//    strcpy(global_data::id[3],settings->value("DeviceName/name4").toByteArray().data());
    ...
    strcpy(global_data::id[0],"CQHC210522003");
...
void MainWindow::on_login_clicked()
{
           ...
           if( !list.isEmpty()){
               USER_UI->Silkworm_Area->addItems(list);
               USER_UI->Silkworm_device->addItems(DB.GetDevice(USER_UI->Silkworm_Area->currentText()));
           }
           list.clear();
           list  = DB.Get_AbNormal_Device();
           if( !list.isEmpty() ){
                USER_UI->Abnormal_Widegt->addItems(list);
           }
