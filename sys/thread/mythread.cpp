    ...
    while( !QThread::currentThread()->isInterruptionRequested() ){
        for( uint8_t i = 0 ; i < DEV ; i++ ){
            ...
            if( global_data::on_line ){
                if( network_Status && global_data::Default_flag){
                   // QThread::sleep(5);
                    if( global_data::temp_flag )
                    {
                        //Update_judgment();
                    ...
                    if(global_data::Send_onine_flag[i])
                        DBT->GetTable(QString(global_data::id[i]));
                    Mythread::mutex.unlock();
                }
            }

            QThread::msleep(Parameters_Times);
        }
    }
...
void Mythread::FromDB_Data()
{
    ...
    while( !QThread::currentThread()->isInterruptionRequested() ){
        ...
        for( uint8_t i = 0 ; i < DEV  ; i++ ){
            ...
            if( global_data::Mutual_exclusion[i] == DataBase){
                From_data.id = QString(global_data::id[i]);
                if( !DBT->toDB() ){
                ...
                QThread::msleep(Equipment_Times);
